  ...
  var Keyboard = function (elemento, opciones) {
      ...
      $.each(language, function (key, val) {
        ...
        if (val.d)
          if (val.d.length == 1)
            if (!divisor)
              keys.push(
                '<button class="btn btn-default char" data-up="' +
                  val.u +
                  '" data-down="' +
                  val.d +
                  '">' +
          ...
          else if (val.d == "divisor") divisor = true;
          else {
            if (!divisor)
              keys.push(
    ...
    deleteChar = function (action, char) {
      ...
      switch (action) {
        case "delete":
          if (pos_i < length) {
            if (pos_e - pos_i == 0) array.splice(pos_i, 1);
            else if (pos_e - pos_i > 0) array.splice(pos_i, pos_e - pos_i);
            else return false;
        ...
        case "enter":
          if (elementoDOM.is("textarea")) {
            if (pos_e < length) {
              array_1 = array
                .slice(0, pos_i)
                .concat(String.fromCharCode(13, 10));
      ...
      value = "";
      for (i = 0; i < array.length; i++) value = value.concat(array[i]);
    ...
    _this.setOpenSpeed = function ($value) {
      if ($value >= 100) optionsKeyboard.open_speed = parseInt($value);
    };
    ...
    _this.setCloseSpeed = function ($value) {
      if ($value >= 100) optionsKeyboard.close_speed = parseInt($value);
    };
