...
var lowLag = new (function () {
  ...
  this.playSoundAudioContext = function (tag) {
    ...
    if (
      (lowLag.currentPlayingTag &&
        lowLag.currentTag &&
        lowLag.currentPlayingTag === lowLag.currentTag) ||
      (tag && lowLag.currentPlayingTag && lowLag.currentPlayingTag === tag)
    ) {
      ...
      if (context !== undefined) {
        ...
        if (context.state === "suspended") {
          context
            ...
            .then(function () {
              lowLag.msg(
                "playSoundAudioContext resume " + lowLag.currentPlayingTag
              );
              return;
            })
            .catch(function (e) {
              lowLag.msg(
                "playSoundAudioContext resume error for " +
                  lowLag.currentPlayingTag +
                  ". Error: " +
                  e
              );
    ...
    var sourceOld = lowLag.currentPlayingTag
    ...
    if (context.state === "running") {
      ...
      lowLag.msg(
        "playSoundAudioContext failed to start, context closed for " + tag
      );
    } else {
      lowLag.msg(
    ...
    if (context !== undefined) {
      ...
      if (context.state === "running") {
        ...
        context
          ...
          .catch(function (e) {
            lowLag.msg(
              "pauseSoundAudioContext suspend error for " +
                tag +
                ". Error: " +
                e
            );
        ...
        lowLag.msg(
          "pauseSoundAudioContext unknown AudioContext.state for " +
            tag +
            ". State: " +
            context.state
        );
  ...
  this.switchSoundAudioContext = function (autoplay) {
    lowLag.msg(
      "switchSoundAudioContext " +
        (autoplay ? "and autoplay" : "and do not autoplay")
    );
  ...
  this.changeSoundAudioContext = function (tag, autoplay) {
    lowLag.msg(
      "changeSoundAudioContext to tag " +
        tag +
        " " +
        (autoplay ? "and autoplay" : "and do not autoplay")
    );
