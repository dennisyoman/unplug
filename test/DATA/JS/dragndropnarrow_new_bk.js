...
var showAnswer = function (boolean) {
  ...
  if (boolean) {
    ...
    containers.each(function (index) {
      var $c = $(this);
      var oriX =
        $c.offset().left / stageRatioReal - deltaContainerX / stageRatioReal;
      var oriW = $c.width() / stageRatioReal;
      var oriY =
        $c.offset().top / stageRatioReal - deltaContainerY / stageRatioReal;
      var oriH = $c.height() / stageRatioReal;
      containerRects[index] = { x: oriX, y: oriY, w: oriW, h: oriH };
      ansArray[index] = [];
    });
        ...
        if (matchGroup || (apOnly && index === 0)) {
          ...
          if (hasAp) {
            ...
            for (var a = 0; a < aps.length; a++) {
              ...
              ansLeft = parseFloat(ap[1]);
              var targetIndex = index;
              if (apOnly) {
                var bestIndex = 0;
                var bestDist = Infinity;
                for (var r = 0; r < containerRects.length; r++) {
                  var rect = containerRects[r];
                  if (!rect) continue;
                  var dx = ansLeft - rect.x;
                  var dy = ansTop - rect.y;
