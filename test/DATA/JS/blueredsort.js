...
var showAnswer = function (boolean) {
  if (boolean) {
    ...
    //
    var rows = $(".contents > div.selected .row span");
    rows.each(function () {
...
var showCombination = function (tar) {
  var rowID = tar.attr("tar");
  var tarRow = $(rowID);
  var red = 0;
  var blue = 0;

  $("#module_wrapper .contents > div.selected .sensor ").each(function () {
    if ($(this).attr("tar") == rowID) {
      if ($(this).attr("fillin") == "red") {
        red++;
      }
      if ($(this).attr("fillin") == "blue") {
        blue++;
      }
    }
  });
  //
  tarRow.find("> span").eq(0).text(red);
  tarRow.find("> span").eq(1).text(blue);
};
...
var showSwifter = function () {
  ...
  if (done) {
    $(".contents > div.selected").find(".swifter").show();
  }
};
...
var moveRow = function (attr1, attr2) {
  ...
  sensorGroup1.each(function (index) {
    ...
    var fillinElem = $(this).html();
    var brother = sensorGroup2.eq(index);
    console.log(fillin);
    console.log(fillinElem);
    $(this).attr("fillin", brother.attr("fillin"));
    $(this).empty().append(brother.html());
    brother.attr("fillin", fillin);
    brother.empty().append(fillinElem);
  });
  rootSoundEffect($show);
  //
  showCombination(sensorGroup1.eq(0));
  showCombination(sensorGroup2.eq(0));
};
...
var resetElem = function (elem) {
  ...
  if (elem.find(".row").length > 0) {
    elem.find(".row").find("> span").empty();
  }
  //swifter
  if (elem.find(".swifter").length > 0) {
    elem.find(".swifter").hide();
  }
  //
  $(".smoke").remove();
  $(".resultIcon").remove();
